...
from app.schemas import AlternativeRegion, RegionInfo
...
def find_alternatives(
    ...
    for region_id, score in all_scores.items():
        if region_id == target.region_id or score <= own_score:
            continue
        other = region_by_id[region_id]
        distance_km = haversine_km(target.위도, target.경도, other.위도, other.경도)
        if distance_km <= MAX_DISTANCE_KM:
            candidates.append(AlternativeRegion(region=other, score=score, distance_km=round(distance_km, 2)))
