...
// 🚀 카카오 로컬 API 실시간 경쟁업체 수 크로스체크
//
// ⚠️ 보안 주의: REST API 키를 프론트엔드 번들에 그대로 두면(환경변수로 감싸도 Vite가
// 빌드 시 인라인시키므로 동일) 브라우저 개발자도구 Network 탭에서 누구나 키를 볼 수
// 있다. 데모/해커톤 단계를 벗어나면 백엔드에 프록시 엔드포인트를 만들어 키를 서버에만
// 두고, 프론트는 그 프록시만 호출하도록 바꿔야 한다. 지금은 원래 코드 구조를 유지한
// 채 하드코딩만 걷어내고 환경변수로 옮겨둔 상태.
const KAKAO_REST_KEY = import.meta.env.VITE_KAKAO_REST_KEY

async function fetchKakaoCompetitorCount(regionName, category) {
  if (!KAKAO_REST_KEY) return "알 수 없음"
  try {
    const query = `${regionName} ${category}`
    ...
    return data.meta?.total_count ?? "알 수 없음"
  } catch (e) {
    console.error("카카오 API 호출 실패, 무시하고 진행", e)
    return "알 수 없음"
  }
}
...
function App() {
  ...
  async function openRegionDetail(regionId, regionName, overrideCategory) {
    ...
    // 공공데이터 기반 경쟁업체 수(market_data.competitors.total_count)가 없거나
    // 0이면, 카카오 로컬 API로 실시간 검색 결과 수를 받아와 보완한다. 리포트
    // 응답을 기다리는 동안 병렬로 호출해두고, 실제로 필요할 때만(공공데이터가
    // 비어있을 때만) 기다려서 쓴다.
    const kakaoCountPromise = fetchKakaoCompetitorCount(regionName, effectiveCategory)

    const reportPromise = fetchReport([regionId], effectiveCategory)
      .then(async (report) => {
        const candidate = report.candidates[0]
        const hasNoCompetitorData =
          !candidate.market_data?.competitors?.total_count || candidate.market_data.competitors.total_count === 0

        if (hasNoCompetitorData) {
          const realTimeCount = await kakaoCountPromise
          if (!candidate.market_data.competitors) candidate.market_data.competitors = {}
          candidate.market_data.competitors.total_count = realTimeCount
          report.report_text += `\n\n📌 [실시간 데이터 크로스체크] 공공데이터 누락을 보완하기 위해 카카오 API로 실시간 확인한 결과, 현재 ${regionName}에는 약 ${realTimeCount}개의 ${effectiveCategory} 장소가 운영 중입니다.`
        }
    ...
    <footer className="data-notice-footer">
      <details>
        ...
        <ul className="data-notice-list">
          ...
          <li>
            일부 지역의 실시간 경쟁업체 수는 카카오(Kakao) 로컬 API 검색 결과로 보완됩니다 — 공공데이터에 값이 없거나
            0으로 집계된 경우에만 참고용으로 사용됩니다.
          </li>
        </ul>
      </details>
      <p className="map-attribution">
        지도 및 장소 검색 정보 일부 제공: 카카오(Kakao). ⓒ Kakao Corp.
      </p>
    </footer>
  )
}
